@section('title', ''.$user->name.'\'s Dashboard &ndash; Application Received | '.env('APP_TITLE'))
                        ...
                        <div class="col-lg-12">
                            <ul class="nav nav-tabs" id="vac-nav-tabs">
                                @foreach($vacancies as $vacancy)
                                    @php $total = \App\Accepting::where('vacancy_id', $vacancy->id)
                                    ->where('isApply', true)->count(); @endphp
                                    <li id="{{$vacancy->id}}">
