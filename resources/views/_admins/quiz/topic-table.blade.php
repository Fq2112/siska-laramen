...
@section('title', 'Quiz Topics Table &ndash; SISKA Admins | SISKA &mdash; Sistem Informasi Karier')
@section('content')
                ...
                <div class="x_panel">
                    <div class="x_title">
                        ...
                        <ul class="nav navbar-right panel_toolbox">
                            ...
                            <li><a onclick="createTopic()" data-toggle="tooltip" title="Create" data-placement="right">
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered">
                            ...
                            @foreach($topics as $topic)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle"><strong>{{$topic->name}}</strong></td>
                                    <td style="vertical-align: middle">{{\Carbon\Carbon::parse($topic->created_at)->format('j F Y')}}</td>
                                    <td style="vertical-align: middle">{{$topic->updated_at->diffForHumans()}}</td>
                                    <td style="vertical-align: middle" align="center">
                                        <a onclick='editTopic("{{$topic->id}}","{{$topic->name}}")'
                                           class="btn btn-warning btn-sm" style="font-size: 16px" data-toggle="tooltip"
                                           title="Edit" data-placement="left">
                                            <i class="fa fa-edit"></i></a>
                                        <a href="{{route('quiz.delete.topics',['id'=>encrypt($topic->id)])}}"
    ...
    <div id="createModal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('quiz.create.topics')}}">
                    {{csrf_field()}}
                    <div class="modal-body">
                        <div class="row form-group">
                            <div class="col-lg-12 has-feedback">
                                <label for="name">Topic <span class="required">*</span></label>
                                <input id="name" type="text" class="form-control" maxlength="191" name="name"
                                       placeholder="Quiz topic" required>
    ...
    <script>
        ...
        function editTopic(id, name) {
            $('#editModalContent').html(
                ...
                '<div class="modal-body">' +
                '<div class="row form-group">' +
                '<div class="col-lg-12 has-feedback"><label for="name' + id + '">Topic ' +
                '<span class="required">*</span></label>' +
                '<input id="name' + id + '" type="text" class="form-control" maxlength="191" ' +
                'placeholder="Quiz topic" value="' + name + '" name="name" required>' +
