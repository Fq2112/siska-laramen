...
@section('title', 'Provinces Table &ndash; SISKA Admins | SISKA &mdash; Sistem Informasi Karier')
@section('content')
                ...
                <div class="x_panel">
                    <div class="x_title">
                        ...
                        <ul class="nav navbar-right panel_toolbox">
                            ...
                            <li><a onclick="createProvince()" data-toggle="tooltip" title="Create"
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered">
                            ...
                            @foreach($provinces as $province)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle"><strong>{{$province->name}}</strong></td>
                                    <td style="vertical-align: middle">{{\Carbon\Carbon::parse($province->created_at)->format('j F Y')}}</td>
                                    <td style="vertical-align: middle">{{$province->updated_at->diffForHumans()}}</td>
                                    <td style="vertical-align: middle" align="center">
                                        <a onclick='editProvince("{{$province->id}}","{{$province->name}}")'
                                           class="btn btn-warning btn-sm" style="font-size: 16px" data-toggle="tooltip"
                                           title="Edit" data-placement="left">
                                            <i class="fa fa-edit"></i></a>
                                        <a href="{{route('delete.provinces',['id'=>encrypt($province->id)])}}"
    ...
    <div id="createModal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('create.provinces')}}">
                    {{csrf_field()}}
    ...
    <script>
        ...
        function editProvince(id, name) {
            $('#editModalContent').html(
                '<form method="post" id="' + id + '" action="{{url('admin/tables/web_contents/provinces')}}/' + id +
                '/update">{{csrf_field()}} {{method_field('PUT')}}' +
                '<div class="modal-body">' +
                '<div class="row form-group">' +
                '<div class="col-lg-12 has-feedback"><label for="name' + id + '">Province ' +
                '<span class="required">*</span></label>' +
                '<input id="name' + id + '" type="text" class="form-control" maxlength="191" ' +
                'value="' + name + '" name="name" required>' +
                '<span class="fa fa-monument form-control-feedback right" aria-hidden="true"></span>' +
                '</div></div></div>' +
                '<div class="modal-footer">' +
                '<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>' +
